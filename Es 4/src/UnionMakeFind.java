...
//we are used ArrayList because for rappresent the union make findset because its simple and efficent in particular operations have O(n)
//complexities.
...
public class UnionMakeFind<T> {     //implenting UNION MAKE FIND
    ...
    int lastId;
    int nodeNumber;
    Hashtable<T, Integer> map;      //used for allow to work in ADT
    List<Integer> parent;           //define all connection of the strcture 
    List<Integer> size;             //used for memorize the number of elements each root
    ...
    public void union(T x, T y) {             //this method its used for implementing "UNION"
      ...
      int yr = find(y);
      System.out.println("Unito x " + xr + " y " + yr + ": " +parent.toString());                        //call find of the second node  and save to the variable yr
